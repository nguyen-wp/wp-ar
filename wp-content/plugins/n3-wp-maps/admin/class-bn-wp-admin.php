...
class MADE_WP_MAPS_Admin {
	...
	public function __madeapp_plugin_row_meta( $links, $file ) {
		...
		if( $file === $plugin_dir.'/nguyen-app.php' || $file === $plugin_dir.'/index.php' || $file === $plugin_dir.'/plugin.php' ){
			...
			$links[] = '<a href="' . esc_url( get_admin_url(null, 'edit.php?post_type=n3_maps&page=n3_maps_settings') ) . '">' . esc_html__( 'Settings', 'duplicate-post' ) . '</a>';
		}
		return $links;
	}
	...
	public function enqueue_styles() {
		...
		wp_register_style( 'wp_maps_admin_main_css',  plugin_dir_url( __FILE__ ) . 'assets/css/main.min.css', array() );
		wp_enqueue_style('wp_maps_admin_main_css');
	...
	public function enqueue_scripts() {
		...
		wp_register_script('wp_maps_admin_main_js', plugin_dir_url( __FILE__ ) . 'assets/js/main.prod.js', array('jquery'));
        wp_enqueue_script('wp_maps_admin_main_js');
