 ...
 *
 * @wordpress-plugin
 * Plugin Name:       @Restore Construction Services
...
define( 'MADE_WP_SERVICES_NICENAME', 'Restore Construction Services' );
define( 'MADE_WP_SERVICES_PREFIX', 'n3_wp_services' );
define( 'MADE_WP_SERVICES_VERSION', '2.5.1' );
define( 'GENERALPUBLIC_WP_SERVICES_CORE_URL', plugin_dir_url( __FILE__ ) ); // Return https://...
define( 'GENERALPUBLIC_WP_SERVICES_CORE_PATH', plugin_dir_path( __FILE__ ) ); // Return /path/to/...
 ...
 */
function deactivate_n3_wp_services() {
	require_once plugin_dir_path( __FILE__ ) . 'includes/class-bn-wp-deactivator.php';
	new MADE_WP_SERVICES_Deactivator();
}

register_activation_hook( __FILE__, 'activate_n3_wp_services' );
register_deactivation_hook( __FILE__, 'deactivate_n3_wp_services' );
 ...
 * @since    1.0.0
 */
function run_n3_wp_services() {

	$plugin = new MADE_WP_SERVICES();
	$plugin->run();
