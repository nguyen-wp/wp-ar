...
class Backup_Database {
     ...
     */
    public function __construct($host, $username, $passwd, $dbName, $charset = 'utf8', $name = null) {
        ...
        if ($name) {
            $this->backupFile = $name . '.sql';
        } else {
            $this->backupFile  = 'backup_'.$this->dbName.'_'.date("Ymd_His", time()).'.sql';
        }
        $this->gzipBackupFile          = defined('GZIP_BACKUP_FILE') ? GZIP_BACKUP_FILE : true;
        $this->disableForeignKeyChecks = defined('DISABLE_FOREIGN_KEY_CHECKS') ? DISABLE_FOREIGN_KEY_CHECKS : false;
        $this->batchSize               = defined('BATCH_SIZE') ? BATCH_SIZE : 10000000; // default 1000 rows
        $this->output                  = '';
    }
