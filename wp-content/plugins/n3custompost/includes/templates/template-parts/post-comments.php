...
<div class="<?php echo esc_attr( $wrapper_class ); ?>" <?php if ( !empty($wrapper_style) ) { ?>style="<?php echo esc_attr($wrapper_style); ?>"<?php } ?>>
	...
	<a href="<?php echo esc_url( get_comments_link() ); ?>"><?php
        if ( get_comments_number() ) {
			...
			echo esc_html( sprintf( _n( '%d Comment', '%d Comments', get_comments_number(), 'n3custompost' ), get_comments_number() ) );
        } else {
            echo esc_html__( 'No comments', 'n3custompost' );
        }
    ?></a>
</div><?php if (isset($attributes['blockDivider']) && $attributes['blockDivider'] != ''){ ?><span class='n3custompost-post-meta-divider'><?php echo esc_attr($attributes['blockDivider']); ?></span><?php } ?>
