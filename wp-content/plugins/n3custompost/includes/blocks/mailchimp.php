...
class MailChimp extends \N3Block\Blocks\AbstractBlock {

    private $mailchimp;

	protected static $blockName = 'n3custompost/mailchimp';

    public function __construct() {

		parent::__construct( self::$blockName );

        add_action( 'wp_ajax_n3custompost_mailchimp_api_key_manage', [ $this, 'mailchimp_api_key_manage'] );

        add_action( 'wp_ajax_n3custompost_subscribe'       , [ $this, 'subscribe' ] );
        add_action( 'wp_ajax_nopriv_n3custompost_subscribe', [ $this, 'subscribe' ] );

        $this->register_mailchimp_blocks();
    }

	public function getLabel() {
		return __('Mailchimp', 'n3custompost');
	}
    ...
    private function register_mailchimp_blocks() {
        ...
        register_block_type(
            'n3custompost/mailchimp',
            array(
                'render_callback' => [ $this, 'render_callback' ]
            )
        );

		$field_email = 'n3custompost/mailchimp-field-email';
        register_block_type(
            $field_email,
            array(
		...
		$field_first_name = 'n3custompost/mailchimp-field-first-name';
        register_block_type(
            $field_first_name,
            array(
		...
		$field_last_name = 'n3custompost/mailchimp-field-last-name';
        register_block_type(
            $field_last_name,
            array(
    ...
    public function block_frontend_assets() {
		...
		wp_enqueue_style(
			self::$blockName,
			n3custompost_get_plugin_url( 'assets/blocks/mailchimp/style' . $rtl . '.css' ),
			[],
		...
		wp_enqueue_script(
            self::$blockName,
            n3custompost_get_plugin_url( 'assets/blocks/mailchimp/frontend.js' ),
            [ 'jquery' ],
            n3custompost()->settings()->getVersion(),
            true
        );
		...
		/*
		 * var N3Block = {"ajax_url":"https:\/\/n3custompost.loc\/wp-admin\/admin-ajax.php"};
		 */
		$inline_script =
			'var N3Block = N3Block || {};' .
    ...
    public function render_callback( $attributes, $content ) {
        ...
        $class      = 'wp-block-n3custompost-mailchimp';
        $block_name = $class;
        ...
        if ( isset( $attributes[ 'ids' ] ) ) {?>
            <div class='<?php echo esc_attr( $class ); ?>'>
                <?php n3custompost_get_template_part( 'mailchimp/mailchimp', $attributes, false, $extra_attr ); ?>
            </div><?php
        } else {?>
            <p><?php echo esc_html__( 'Select at least one MailChimp list.', 'n3custompost' ); ?></p><?php
        }
    ...
    public function render_mailchimp_field_email( $attributes ) {
        ob_start();?>
        <?php n3custompost_get_template_part( 'mailchimp/field-email', $attributes, false ); ?><?php
    ...
    public function render_mailchimp_field_first_name( $attributes ) {
        ...
        if ( ! isset( $attributes[ 'label' ] ) ) {
            $attributes[ 'label' ] = __( 'First name', 'n3custompost' );
        }

        ob_start();?>
        <?php n3custompost_get_template_part( 'mailchimp/field-first-name', $attributes, false, $extra_attr ); ?><?php
    ...
    public function render_mailchimp_field_last_name( $attributes ) {
        ...
        if ( ! isset( $attributes[ 'label' ] ) ) {
            $attributes[ 'label' ] = __( 'Last name', 'n3custompost' );
        }

        ob_start();?>
        <?php n3custompost_get_template_part( 'mailchimp/field-last-name', $attributes, false, $extra_attr ); ?><?php
    ...
    public function mailchimp_api_key_manage() {
        ...
        if ( ! wp_verify_nonce( $nonce, 'n3custompost_nonce_mailchimp_api_key' ) ) {
            wp_send_json_error();
        }
        ...
        if ( $option == 'save' || $option == 'sync' ) {
            if ( ! empty( $api_key ) ) {
                update_option( 'n3custompost_mailchimp_api_key', $api_key );
            ...
            delete_option( 'n3custompost_mailchimp_api_key' );
    ...
    public function subscribe() {
        ...
        if ( empty( $email ) || ! is_email( $email ) ) {
            wp_send_json_error( __('Email is required.', 'n3custompost') );
        }

		if ( empty( $_POST[ 'data' ][ 'list_ids' ] ) ) {
            wp_send_json_error( __('An invalid Mailchimp list was provided.', 'n3custompost') );
        }
        ...
        $api_key = get_option( 'n3custompost_mailchimp_api_key' );
