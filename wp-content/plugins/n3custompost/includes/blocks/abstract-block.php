...
abstract class AbstractBlock {
	...
	public function hasBlock() {
		...
		return apply_filters( 'n3custompost/blocks/has_block', $has_block, $this->blockName );
	}
	...
	public function isDisabled() {
		...
		n3custompost_maybe_add_option( $this->getDisabledOptionKey(), false, true );

		return apply_filters( 'n3custompost/blocks/is_disabled', $disabled, $this->blockName );
	}
		...
		if ( $block['blockName'] === static::$blockName ) {
			...
			if ( current_user_can('manage_options') ) {
				...
				$block_content .=  sprintf(
					...
					__( '%1$s block is disabled in plugin settings. <a href="%2$s">Manage Blocks</a>', 'n3custompost'),
					esc_html( $this->getLabel() ),
					esc_url( n3custompost()->settingsPage()->getTabUrl('blocks') )
				);
				$block_content .= '</p>';
			}
		}
