...
class Countdown extends \N3Block\Blocks\AbstractBlock {

	protected static $blockName = 'n3custompost/countdown';
    ...
    public function __construct() {
		...
		if ( $this->isEnabled() ) {
			...
			wp_register_script(
				'jquery-plugin',
				n3custompost_get_plugin_url( 'vendors/jquery.countdown/jquery.plugin.min.js' ),
				[ 'jquery' ],
				'1.0',
				true
			);
			...
			wp_register_script(
				'jquery-countdown',
				n3custompost_get_plugin_url( 'vendors/jquery.countdown/jquery.countdown.min.js' ),
			...
			if ( $locale_prefix != '' ) {
				...
				if ( file_exists( n3custompost_get_plugin_path( $locale_path ) ) ) {
					wp_register_script(
						'jquery-countdown-' . $locale_prefix,
						n3custompost_get_plugin_url( $locale_path ),
						[ 'jquery-countdown' ],
						'2.1.0',
						true
					);
	...
	public function getLabel() {
		return __('Countdown', 'n3custompost');
	}
    ...
    public function block_frontend_assets() {
		...
		wp_enqueue_style(
			self::$blockName,
			n3custompost_get_plugin_url( 'assets/blocks/countdown/style' . $rtl . '.css' ),
			[],
		...
		wp_enqueue_script(
            self::$blockName,
            n3custompost_get_plugin_url( 'assets/blocks/countdown/frontend.js' ),
    ...
    public function render_callback( $attributes, $content ) {
		...
		$block_name = 'wp-block-n3custompost-countdown';
		$class      = $block_name;
		...
		//Color style & class
		n3custompost_custom_color_style_and_class( $content_style, $content_class, $attributes, 'color', $is_back_end );

		try {
			$target_date = new \DateTime( $attributes['dateTime'] );
		} catch ( Exception $e ) {
			return esc_html__( 'Invalid date.', 'n3custompost' );
		}
