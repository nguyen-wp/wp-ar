...
class AssetsOptimization {
	...
	*/
	public function __construct() {
		...
		n3custompost_maybe_add_option( 'n3custompost_load_assets_on_demand', false, true );
		n3custompost_maybe_add_option( 'n3custompost_move_css_to_head', false, true );
		...
		add_filter( 'n3custompost/html/output', array( $this, 'filter_css' ) );
		add_filter( 'n3custompost/html/output', array( $this, 'filter_test' ) );
	...
	public function buffer_end( $html ) {
		...
		$html = apply_filters( 'n3custompost/html/output', $html );
	...
	public function filter_test( $html ) {
		...
		if ( $this->alreadyoptimized ) {
			$html .= PHP_EOL . '<!-- n3custompost optimized -->';
		}
	...
	public function filter_css( $html ) {
		...
		$optimize_more = apply_filters( 'n3custompost/optimize/assets', [] );

		/*
		 * Blocks array example:
		 * ["n3custompost/anchor"] => object(N3Block\Blocks\Anchor) {}
		 *
		 * Style tag example:
		 * <link rel='stylesheet' id='n3custompost/banner-css'  href='https://.../wp-content/plugins/n3custompost/assets/blocks/banner/style.css' type='text/css' media='all' />
		 */
		...
		if ( ! empty( $css ) ) {
			...
			// Debug
			$css_scope = PHP_EOL . '<!-- n3custompost styles -->' . PHP_EOL . "\t";
	...
	{
    	return get_option( 'n3custompost_load_assets_on_demand', false );
	}

    public function move_css_to_head()
	{
    	return get_option( 'n3custompost_move_css_to_head', false );
	}
