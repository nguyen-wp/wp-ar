...
class VersionControl {
    ...
    {
        return get_option('n3custompost_db_version', '0.0.0');
    }

    protected function setCurrentDatabaseVersion($version)
    {
        update_option('n3custompost_db_version', $version);
    }

    protected function addVersionToHistory($version)
    {
        $versionHistory = get_option('n3custompost_db_version_history', []);

        if (!in_array($version, $versionHistory)) {
            $versionHistory[] = $version;
            update_option('n3custompost_db_version_history', $versionHistory);
        }
    }
}
