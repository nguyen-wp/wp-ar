...
class RestAPI {
	...
	protected $_namespace = 'n3custompost/v1';
	...
	public function __construct( ) {
		...
		$this->remote_template_library_url = defined( 'N3BLOCK_REMOTE_TEMPLATE_LIBRARY_URL' ) ?
			N3BLOCK_REMOTE_TEMPLATE_LIBRARY_URL : 'https://nguyenpham.pro';
	...
	public function get_remote_templates() {
		...
		if ($cache == 'cache'){
			$templates_data = get_transient( 'n3custompost_templates_response_data' ); //Get Cache response
		} elseif ($cache == 'refresh') {
			delete_transient( 'n3custompost_templates_response_data' ); //Delete cache data
		}
		...
		if ( $templates_data == false || empty( $templates_data ) ) {
			...
			$response = wp_remote_get(
				$this->remote_template_library_url . "/wp-json/n3custompost-templates-server/v1/get_templates",
				array(
					'timeout' => 15,
				)
			);
			...
			if ( is_wp_error( $response ) ) {
				...
				if ( json_last_error() === JSON_ERROR_NONE && $templates_data ) {
					...
					set_transient( 'n3custompost_templates_response_data', $templates_data, 24 * HOUR_IN_SECONDS ); //Cache response
	...
	public function get_terms($object) {
		$taxonomy_name = n3custompost_recursive_sanitize_array( wp_unslash( $_GET['taxonomy_name'] ) );
